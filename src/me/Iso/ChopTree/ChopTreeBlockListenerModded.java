...
public class ChopTreeBlockListenerModded
implements Listener
{
	public static ChopTree plugin;
	private int theLog = 243;
	private int theLeave = 242;
	public static Player pubplayer = null;

	public ChopTreeBlockListenerModded(ChopTree instance)
	{
		plugin = instance;
	}
	...
	public void onBlockBreak(BlockBreakEvent event)
	{
		...
		List<String> other = plugin.getConfig().getStringList("Modded");
		Block block = event.getBlock();
		for(String m : other){
			String[] split = m.split(":");
			if (split.length == 2){
				theLog = (Integer.parseInt(split[0]));
				theLeave = (Integer.parseInt(split[1]));
				if (block.getType().getId() == theLog)
				{
	...
	public void getBlocksToChop(Block block, Block highest, List<Block> blocks)
	{
		while (block.getY() <= highest.getY())
		{
			...
			if ((blocks.contains(block.getRelative(BlockFace.UP))) || (block.getRelative(BlockFace.UP).getType().getId() != theLog)) {
				break;
			}
	...
	{
		if ((!blocks.contains(other)) && (other.getType().getId() == theLog)) {
	...
	{
		while (block.getRelative(BlockFace.UP).getType().getId() == theLog) {
			block = block.getRelative(BlockFace.UP);
		}
		return block;
	}
	...
	public boolean isTree(Block block, Player player, Block first)
	{
		...
		int counter = 0;
		if (block.getRelative(BlockFace.UP).getType().getId() == theLeave) {
			counter++;
		}
		if (block.getRelative(BlockFace.DOWN).getType().getId() == theLeave) {
			counter++;
		}
		if (block.getRelative(BlockFace.NORTH).getType().getId() == theLeave) {
			counter++;
		}
		if (block.getRelative(BlockFace.EAST).getType().getId() == theLeave) {
			counter++;
		}
		if (block.getRelative(BlockFace.SOUTH).getType().getId() == theLeave) {
			counter++;
		}
		if (block.getRelative(BlockFace.WEST).getType().getId() == theLeave) {
			counter++;
		}
		if (counter >= 2) {
			return true;
		...
		for (int y = -plugin.leafRadius; y < plugin.leafRadius + 1; y++) {
			for (int x = -plugin.leafRadius; x < plugin.leafRadius + 1; x++) {
				for (int z = -plugin.leafRadius; z < plugin.leafRadius + 1; z++)
				{
					...
					if (target.getType().getId() == theLeave) {
	...
	public void moveDownLogs(Block block, List<Block> blocks, World world, Player player)
	{
		...
		for (int counter = 0; counter < blocks.size(); counter++)
		{
			...
			if ((down.getType() == Material.AIR) || (down.getType().getId() == theLeave))
			{
				down.setTypeId(theLog);
	...
	public void moveLeavesDown(List<Block> blocks)
	{
		...
		for (Block block : blocks) {
			for (int y = -plugin.leafRadius; y < plugin.leafRadius + 1; y++) {
				for (int x = -plugin.leafRadius; x < plugin.leafRadius + 1; x++) {
					for (int z = -plugin.leafRadius; z < plugin.leafRadius + 1; z++) {
						if ((block.getRelative(x, y, z).getType().getId() == theLeave) && (!leaves.contains(block.getRelative(x, y, z)))) {
		...
		for (Block block : leaves) {
			if (((block.getRelative(BlockFace.DOWN).getType().equals(Material.AIR)) || (block.getRelative(BlockFace.DOWN).getType().getId() == theLeave)) && ((block.getRelative(BlockFace.DOWN, 2).getType().equals(Material.AIR)) || (block.getRelative(BlockFace.DOWN, 2).getType().getId() == theLeave) || (block.getRelative(BlockFace.DOWN, 2).getType().getId() == theLog)) && ((block.getRelative(BlockFace.DOWN, 3).getType().equals(Material.AIR)) || (block.getRelative(BlockFace.DOWN, 3).getType().getId() == theLeave) || (block.getRelative(BlockFace.DOWN, 3).getType().getId()==theLog)))
			{
	...
	{
		if (block.getRelative(BlockFace.UP).getType().getId() == theLog) {
			return false;
		}
	...
	{
		if (block.getRelative(BlockFace.NORTH).getType().getId() == theLog) {
			return true;
		}
		if (block.getRelative(BlockFace.NORTH_EAST).getType().getId() == theLog) {
			return true;
		}
		if (block.getRelative(BlockFace.EAST).getType().getId() == theLog) {
			return true;
		}
		if (block.getRelative(BlockFace.SOUTH_EAST).getType().getId() == theLog) {
			return true;
		}
		if (block.getRelative(BlockFace.SOUTH).getType().getId() == theLog) {
			return true;
		}
		if (block.getRelative(BlockFace.SOUTH_WEST).getType().getId() == theLog) {
			return true;
		}
		if (block.getRelative(BlockFace.WEST).getType().getId() == theLog) {
			return true;
		}
		if (block.getRelative(BlockFace.NORTH_WEST).getType().getId() == theLog) {
			return true;
		}
		return false;
	}
